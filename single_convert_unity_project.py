...
def main():
    ...
    # ========================================================================
    # Step 0: .gitignore 가져오기
    # ========================================================================
    
    logger.info(f"\n[STEP 0] Fetching .gitignore for type '{project_type}'...")
    gitignore_content = gitignore_fetcher.fetch_gitignore(project_type)
    
    if not gitignore_content:
        logger.error(f"Failed to fetch .gitignore for type '{project_type}'. Cannot proceed.")
        sys.exit(1)
    ...
    success, msg = utils.init_and_commit_project(
        project_path, 
